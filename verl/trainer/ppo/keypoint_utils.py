...
def normalize_dataframe_keypoints(
    df: pd.DataFrame,
    extra_info_col: str = "extra_info",
    keypoints_col: Optional[str] = None,
    inplace: bool = False,
) -> pd.DataFrame:
    """Normalize keypoints after Parquet round-trip so they are Python lists.

    - Ensures extra_info['keypoints'] is a list (not np.ndarray) per row.
    - If `keypoints_col` is provided, ensures that column's entries are lists.
    """
    if not inplace:
        df = df.copy()

    if extra_info_col in df.columns:
        def _fix_info(x):
            # Sequence of info dicts
            if isinstance(x, (list, tuple, np.ndarray)):
                fixed = []
                for d in x:
                    if isinstance(d, dict) and "keypoints" in d:
                        kp = d["keypoints"]
                        # coerce list-of-arrays as well
                        if isinstance(kp, (list, tuple, np.ndarray)):
                            kp = [_coerce_to_list(e) for e in kp] if isinstance(kp, (list, tuple)) else _coerce_to_list(kp)
                        else:
                            kp = _coerce_to_list(kp)
                        d = {**d, "keypoints": kp if isinstance(kp, list) else [kp] if kp is not None else []}
                    fixed.append(d)
                return fixed
            # Single info dict
            if isinstance(x, dict) and "keypoints" in x:
                kp = x["keypoints"]
                if isinstance(kp, (list, tuple, np.ndarray)):
                    kp = [_coerce_to_list(e) for e in kp] if isinstance(kp, (list, tuple)) else _coerce_to_list(kp)
                else:
                    kp = _coerce_to_list(kp)
                x = {**x, "keypoints": kp if isinstance(kp, list) else [kp] if kp is not None else []}
            return x

        df[extra_info_col] = df[extra_info_col].apply(_fix_info)

    if keypoints_col is not None and keypoints_col in df.columns:
        def _fix_kp(x):
            if isinstance(x, (list, tuple, np.ndarray)):
                return [_coerce_to_list(e) for e in x] if isinstance(x, (list, tuple)) else _coerce_to_list(x)
            return _coerce_to_list(x)

        df[keypoints_col] = df[keypoints_col].apply(_fix_kp)
